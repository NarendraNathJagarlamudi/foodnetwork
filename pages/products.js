...
export default function ProductsPage({
  ...
  const [categories, setCategories] = useState(Object.keys(categoriesWithIds));
      ...
      <Center>
        ...
        <DropdownContainer isVisible={isDropdownVisible}>
          ...
          >
            ...
            {categories.map((cat) => (
              <option key={cat} value={categoriesWithIds[cat]}>
                {cat}
              </option>
            ))}
          </Dropdown>
...
export async function getServerSideProps() {
  ...
  const products = await Product.find({ expiryDate: { $gte: today } }, null, {
    sort: { _id: -1 },
  });
  ...
  const categoriesWithIds = categories.reduce((acc, category) => {
    acc[category.name] = category._id.toString(); // Convert ObjectId to string
    return acc;
  }, {});
  ...
  return {
    props: {
      ...
      categoriesWithIds: JSON.parse(JSON.stringify(categoriesWithIds)),
    },
  };
}
